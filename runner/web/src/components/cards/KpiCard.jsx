...
export default function KpiCard({ title, value, subValue, theme = "dark" }) {
  ...
  const bgClass = isDark ? "bg-gray-800 border-gray-700 shadow-md" : "bg-white border-gray-300 shadow-sm";
  ...
  return (
    <div className={`p-4 rounded-lg border ${bgClass} transition-colors duration-300`}>
      ...
      {subValue && (
        <p className={`text-xs mt-1 ${subValueClass} transition-colors duration-300`}>{subValue}</p>
      )}
    </div>
  );
}
